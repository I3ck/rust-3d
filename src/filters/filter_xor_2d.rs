...
impl<F1, F2> FilterXOR2D<F1, F2> where
    F1: IsFilter2D,
    F2: IsFilter2D {

    /// Creates a new FilterXOR2D from two other IsFilter2D
    pub fn build(filter1: F1, filter2: F2) -> Self {
        FilterXOR2D {filter1: Box::new(filter1), filter2: Box::new(filter2)}
    }
}

impl<F1, F2> IsFilter2D for FilterXOR2D<F1, F2> where
    F1: IsFilter2D,
    F2: IsFilter2D {
